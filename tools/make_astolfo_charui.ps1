...
$selectSource = Join-Path $cache 'chara_400400b_2.png'
$faceSource = Join-Path $cache 'command_400400.png'

New-Item -ItemType Directory -Force $cache,$charui,$character | Out-Null
if (-not (Test-Path $selectSource)) {
    Invoke-WebRequest 'https://static.atlasacademy.io/JP/CharaGraph/400400/400400b@2.png' -OutFile $selectSource
}
if (-not (Test-Path $faceSource)) {
    Invoke-WebRequest 'https://static.atlasacademy.io/JP/Servants/Commands/400400/card_servant_1.png' -OutFile $faceSource
}
Copy-Item $selectSource (Join-Path $charui 'char_select_char_name.png') -Force
Copy-Item $selectSource (Join-Path $character 'select_bg_astolfo.png') -Force
...
$face.Dispose()
Write-Output 'Astolfo charui: listo'
