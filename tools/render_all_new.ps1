...
param([ValidateSet("list","listdeep","probe","check","render","measure","save","debug")][string]$Mode="list",[string]$Only="")
...
$forms=@(
 ...
 @{id="2800100";name="Oberon";    modChar="$R\OberonPretender\OberonPretender\character"; dest="frames"},
 @{id="2800110";name="Oberon Winter"; modChar="$R\OberonPretender\OberonPretender\character"; dest="frames_winter"; animSrc="$ex\2800100_anim\Animator\chr\chr.fbx"},
 @{id="2800120";name="Oberon Vortigern"; modChar="$R\OberonPretender\OberonPretender\character"; dest="frames_vortigern"; animSrc="$ex\2800100_anim\Animator\chr\chr.fbx"}
)
foreach($f in $forms){
 $base="$ex\$($f.id)_anim\Animator"
 if(-not(Test-Path $base)){ $base="$ex\$($f.id)_anim\FBX_Animator" }
 if(Test-Path "$base\chr\chr.fbx"){ $f.src="$base\chr"; $f.fbx="chr.fbx" }
 else { $f.src="$base\model"; $f.fbx="model.fbx" }
 $f.tex="$bn\$($f.id).png"
 if(-not $f.ContainsKey("dest")){ $f.dest="frames" }
}
...
function Stage($f){
 ...
 Remove-Item "$rp\anim.fbx" -Force -ErrorAction SilentlyContinue
 if(-not(Test-Path "$($f.src)\$($f.fbx)")){ Write-Output "  [X] falta $($f.src)\$($f.fbx)"; return $false }
 Copy-Item "$($f.src)\$($f.fbx)" "$rp\chr.fbx" -Force
 if($f.ContainsKey("animSrc")){ Copy-Item $f.animSrc "$rp\anim.fbx" -Force }
...
function SetPass($p){ $g=[IO.File]::ReadAllText("$rp\render.gd"); $g=$g -replace 'const PASS := "(measure|save|list|listdeep|probe|check|debug|faceexp)"',"const PASS := `"$p`""; [IO.File]::WriteAllText("$rp\render.gd",$g,(New-Object System.Text.UTF8Encoding($false))) }
...
if($Mode -eq "listdeep"){ SetPass "listdeep"; foreach($f in $forms){ Write-Output "=== RIG $($f.id) $($f.name) ==="; if(Stage $f){ & $md --path $rp 2>&1 | Select-String -Pattern "MESH:|BONE:|BLENDSHAPE:|DONE" | ForEach-Object{$_.Line} } }; exit 0 }
if($Mode -eq "check"){ SetPass "check"; foreach($f in $forms){ Write-Output "=== CHECK $($f.id) $($f.name) ==="; if(Stage $f){ & $md --path $rp 2>&1 | Select-String -Pattern "MODEL|SCALE|DONE|ERROR" | ForEach-Object{$_.Line} } }; exit 0 }
