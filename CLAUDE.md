...
Custom character mods for **Slay the Spire 2** (Early Access, Godot 4 / C#), building **Fate/Grand Order Servants** as playable characters. ("Smash" in the repo name is a pun on Mash, the first character — no FGO Servant is literally named Smash.) Mods target the **MAIN public branch** of the game, currently **v0.107.1**, with **BaseLib pinned to 3.3.0** (the pin must match the BaseLib loaded in the game exactly or the mod won't load).
...
All 10 FGO mods are ported to v0.107.1, compile green, and are published to Steam Workshop (see STATUS.md for IDs). `CoopDeterminismPatch/` is an auxiliary Harmony patch, not a character.
...
| `CoopDeterminismPatch/` | `CoopDeterminismPatch` | — (Harmony patch) | fixes co-op desync of the 3rd-party Ultraman mod; **needs `Ultraman.dll` installed to compile** |
...
- **Deploy real = Steam Workshop** (appid `2868840`): el usuario JUEGA suscrito a los items de Workshop; la `mods/` del juego se mantiene **FGO-limpia**. Publicar = re-subir con `tools/workshop_upload.ps1` desde `dist/` y que el usuario re-sincronice Steam. **Nunca dejes un mod FGO instalado local Y suscrito en Workshop con el mismo id** → id duplicado → crash.
...
- **`CoopDeterminismPatch`** referencia `Ultraman.dll` (`$(ModsPath)Ultraman/`): **no compila sin el mod Ultraman instalado** en el juego. El usuario no lo tiene → ese proyecto puede quedar sin buildear; no bloquea a los demás.
...
- Game install: `G:\SteamLibrary\steamapps\common\Slay the Spire 2` (moved off C: — the old `C:\Program Files (x86)\Steam\...` path is stale; the cracked `F:\Games\...v0.103.3` install is discarded). BaseLib + ModConfig are installed there; the BaseLib version in `mods/` must EXACTLY match the csproj pin (**3.3.0**) or the mod won't load.
